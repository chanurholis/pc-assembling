...
class Mypc extends CI_Controller
{
    ...
    public function export_excel($id)
    {
        if ($this->session->userdata('status') == NULL) {
            ...
            foreach ($result as $data) {
                ...
                $excel->setActiveSheetIndex(0)->setCellValue('G' . $numrow, $data->brand_storage . " " . $data->nama_storage . " " . $data->type_storage . " " . $data->kapasitas_storage . $data->satuan);
    ...
    public function search_mypc()
    {
        if ($this->session->userdata('status') == NULL) {
            ...
            $data['judul'] = 'My PC';
            $data['id'] = $this->db->get('rakit')->row();
            $data['rakit'] = $this->M_mypc->get_keyword_mypc($keyword, $where)->result();
            $this->load->view('partials/header', $data);
            $this->load->view('partials/sidebar_admin');
            $this->load->view('search_mypc', $data);
            $this->load->view('partials/footer');
        }
    }
    ...
    public function export_excel_filter()
    {
        if ($this->session->userdata('status') == NULL) {
            ...
            $result = $this->M_mypc->get_keyword_mypc($keyword, $where)->result();

            foreach ($result as $r) :

                echo "<li>" . $r->no_indeks . "</li>";
                echo "<li>" . $r->institusi . "</li>";
                echo "<li>" . $r->pengguna . "</li>";
                echo "<li>" . $r->brand_processor . "</li>";
                echo "<li>" . $r->user . "</li>";
                echo "<li>" . $r->user . "</li>";
