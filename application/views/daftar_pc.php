        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid">
                        ...
                        <tr>
                            <div class="input-group">
                                <form method="post" action="<?= base_url('home/pencarian') ?>">
                                    <input type="text" class="form-control col-7 rounded text-secondary font-weight-bold" name="keyword" placeholder="Pencarian">
                                    <div class="input-group-append">
                                        <button type="submit" class="input-group-text rounded"><a href=""><i class=" fas fa-search text-dark"></i></a></button>
                                    </div>
                                </form>
                                <form action="<?= base_url('home/app') ?>">
                                    <button class="btn btn-primary float-right ml-3"> <i class="fas fa-plus"></i> Tambah</button>
                                </form>
                                <form action="<?= base_url('home/export') ?>" method="post">
                                    <button type="submit" class="btn btn-success ml-3">
                                        Export Data <i class="fas fa-file-export"></i>
                                    </button>
                                </form>
                            </div>
                        </tr>
                ...
                <div class="border-0">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="font-weight-bold text-secondary"><?= $r->nama_pc ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->processor ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->motherboard ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->ram ?></td>
                                <th class="font-weight-bold text-secondary"><?= $r->hardisk ?></th>
                                <td class="font-weight-bold text-secondary"><?= $r->ssd ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->casing ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->vga ?></td>
                                <th class="font-weight-bold text-secondary"><?= $r->psu ?></th>
                                <td class="font-weight-bold text-secondary"><?= $r->keyboard ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->mouse ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->monitor ?></td>
                                <td class="font-weight-bold text-secondary"><?= $r->pengguna ?></td>
                                <td class="text-center"><a onclick="confirm('Hapus Data?')" style="height:30px; width:90px; font-size:15px; padding:3px;" class="btn btn-danger mb-1" href="<?= base_url('home/hapus/') . $r->id ?>"><i class="fas fa-trash-alt text-center"></i> Hapus</a> <a style="height:30px; width:90px; font-size:15px; padding:3px;" class="btn btn-info" href=""><i class="fa fa-pencil-alt text-center"></i> Ubah</a></td>
                            </tr>
                        </tbody>
                        <?php
                        }
                    ...
                    <a href="<?= base_url('home') ?>" class="btn btn-secondary float-right"><i class="far fa-arrow-alt-circle-left text-light"></i> Kembali</a>
                </div>
            </div>
        </div>
    </div>
